...
class SQLAlchemyStoryRepository(StoryRepository):
    ...
    def get_by_user_id(self, user_id: int, skip: int = 0, limit: int = 100) -> List[StoryEntity]:
        models = (
    ...
    def save(self, story: StoryEntity) -> StoryEntity:
        if story.id:
            model = self.session.query(StoryModel).filter(StoryModel.id == story.id).first()
            if model:
    ...
    def count_by_user_id(self, user_id: int) -> int:
        return self.session.query(StoryModel).filter(StoryModel.user_id == user_id).count()
...
class SQLAlchemyMessageRepository(MessageRepository):
    ...
    def get_by_id(self, message_id: int) -> Optional[MessageEntity]:
        model = self.session.query(MessageModel).filter(MessageModel.id == message_id).first()
        if model:
            return message_model_to_entity(model)
        return None
    
    def get_by_story_id(self, story_id: int, skip: int = 0, limit: int = 100) -> List[MessageEntity]:
        models = (
    ...
    def delete_by_story_id(self, story_id: int) -> int:
        count = self.session.query(MessageModel).filter(MessageModel.story_id == story_id).delete()
        self.session.commit()
        return count
    
    def count_by_story_id(self, story_id: int) -> int:
        return self.session.query(MessageModel).filter(MessageModel.story_id == story_id).count()
...
class SQLAlchemySnippetRepository(SnippetRepository):
    ...
    def get_by_id(self, snippet_id: int) -> Optional[SnippetEntity]:
        model = self.session.query(SnippetModel).filter(SnippetModel.id == snippet_id).first()
        if model:
            return snippet_model_to_entity(model)
        return None
    
    def get_by_story_id(self, story_id: int, include_archived: bool = False) -> List[SnippetEntity]:
        query = self.session.query(SnippetModel).filter(SnippetModel.story_id == story_id)
        if not include_archived:
            query = query.filter(SnippetModel.is_active == True)
        models = query.all()
        return [snippet_model_to_entity(m) for m in models]
    
    def get_by_user_id(self, user_id: int, include_archived: bool = False) -> List[SnippetEntity]:
    ...
    def save(self, snippet: SnippetEntity) -> SnippetEntity:
        if snippet.id:
            model = self.session.query(SnippetModel).filter(SnippetModel.id == snippet.id).first()
            if model:
