    ...
    def execute(self, input_dto: CreateStoryInput) -> CreateStoryOutput:
        ...
        if input_dto.age_range:
            try:
                ...
                raise ValidationError("age_range", f"Invalid age range: {input_dto.age_range}")
        ...
        return CreateStoryOutput(
            ...
            current_phase=saved.current_phase.value if isinstance(saved.current_phase, Phase) else saved.current_phase,
            age_range=saved.age_range.value if saved.age_range else None,
            status=saved.status.value if isinstance(saved.status, StoryStatus) else saved.status,
        )
...
class GetStoryUseCase:
    ...
    def execute(self, input_dto: GetStoryInput) -> StoryDetailOutput:
        ...
        return StoryDetailOutput(
            ...
            current_phase=story.current_phase.value if isinstance(story.current_phase, Phase) else story.current_phase,
    ...
    def execute(self, input_dto: ListStoriesInput) -> List[StoryListItemOutput]:
        ...
        return [
            StoryListItemOutput(
                id=s.id,
                title=s.title,
                current_phase=s.current_phase.value if isinstance(s.current_phase, Phase) else s.current_phase,
                status=s.status.value if isinstance(s.status, StoryStatus) else s.status,
