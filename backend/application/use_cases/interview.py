...
class ProcessChatUseCase:
    ...
    def execute(self, input_dto: ProcessChatInput) -> ProcessChatOutput:
        ...
        chat_messages = [
            ChatMessage(role=m.role.value if isinstance(m.role, MessageRole) else m.role, 
                       content=m.content)
            for m in history
        ]
...
class AdvancePhaseUseCase:
    ...
    def execute(self, input_dto: AdvancePhaseInput) -> AdvancePhaseOutput:
        ...
        return AdvancePhaseOutput(
            previous_phase=str(previous_phase.value) if isinstance(previous_phase, Phase) else str(previous_phase),
            current_phase=str(story.current_phase.value) if isinstance(story.current_phase, Phase) else str(story.current_phase),
