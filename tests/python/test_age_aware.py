...
def test_age_aware_phases():
    ...
    for age_num, age_key, expected_phases in age_ranges:
        ...
        assert route.should_advance(
            age_num
        ), f"Should advance from AGE_SELECTION on '{age_num}'"

        # Verify age range was set
        assert (
            route.age_range == age_key
        ), f"Age range should be {age_key}, got {route.age_range}"

        # Verify phase order matches expected
        assert (
            route.phase_order == expected_phases
        ...
        print(
            f"   ✅ Phases: {', '.join(route.phase_order[2:])}"
        )  # Skip GREETING and AGE_SELECTION
...
def test_route_metadata():
    ...
    assert (
        "chronologically" in info["presentation"].lower()
    ), "Presentation should mention chronological organization"
...
def test_phase_instructions():
    ...
    assert (
        "1-5" in instruction or "1" in instruction and "5" in instruction
    ), "Should mention age options 1-5"
    assert (
        "Under 18" in instruction or "under 18" in instruction
    ), "Should mention 'Under 18' option"
